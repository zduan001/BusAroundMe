...
namespace ConsoleTestProj
{
    ...
    /// <summary>
    ///This is a test class for _019_InsertIntervalsTest and is intended
    ///to contain all _019_InsertIntervalsTest Unit Tests
    ///</summary>
    [TestClass()]
    public class _019_InsertIntervalsTest
    {
        ...
        /// <summary>
        ///A test for InsertIntervals
        ///</summary>
        [TestMethod()]
        public void InsertIntervalsTest()
        {
            _019_InsertIntervals target = new _019_InsertIntervals(); // TODO: Initialize to an appropriate value
            
            List<Interval> intervals = new List<Interval>();
            Interval newInterval = new Interval(5, 7);
            List<Interval> actual = target.InsertIntervals(intervals, newInterval);
            Assert.AreEqual(1, actual.Count);
            Assert.AreEqual(newInterval.Start, actual[0].Start);
            Assert.AreEqual(newInterval.End, actual[0].End);

            intervals = new List<Interval>() { new Interval(1, 5) };
            newInterval = new Interval(2, 3);
            actual = target.InsertIntervals(intervals, newInterval);
            Assert.AreEqual(1, actual.Count);
            ...
            intervals = new List<Interval>(){ new Interval(1, 4), new Interval(6, 6), new Interval(8, 10) };
